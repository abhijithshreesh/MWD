...
class LdaGenreTag(GenreTag):
    ...
    def get_lda_data(self, genre):
         ...
         tag_count_df = genre_data_frame.groupby(['movieid'])['tag'].apply(lambda x: ','.join(x)).reset_index()
         movie_tag_dict = dict(zip(tag_count_df.movieid, tag_count_df.tag))
         count_dict = {movie: self.get_tag_count(tags.split(',')) for movie, tags in
                           list(movie_tag_dict.items())}
         tag_df = self.get_model_weight(count_dict, {}, genre_data_frame, "TF")
         tag_df["value"] = pd.Series([count_dict.get(movieid, 0).get(tag, 0) for index, tag, movieid
         ...
         genre_tag_freq = temp_df.pivot_table(index='moviename', columns='tag', values='value')
